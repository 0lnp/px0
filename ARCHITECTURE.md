...
- Math and Diagrams, Lazily: goldmark marks `$...$` and `$$...$$` spans with GitHub's delimiter rules (`mathParser`), and ` ```math ` fences become display math; the TeX itself stays escaped in `span.md-math`. The browser renders those spans with the vendored KaTeX (`web/lib/katex/`, `throwOnError: false`), and `web/src/mermaid.js` swaps ` ```mermaid ` fences for diagrams from the vendored Mermaid (`web/lib/mermaid/`), re-rendering on theme changes and falling back to the fenced source with an error note when a diagram cannot be drawn. Both libraries load through a computed dynamic `import()` on the first document that needs them, so a preview without math or diagrams fetches neither; `/static/lib/` serves the vendored trees with an immutable cache header.
