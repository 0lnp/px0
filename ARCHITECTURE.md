...
7. Git Awareness (`git.go`): Shells out to the `git` binary (`git status --porcelain=v2 -z`, `git diff`, `git rev-parse`) — no Go git library, no CGO, read-only. Detection and per-file status run inside the same async `Build()` lane as indexing, off the sub-millisecond boot path, and are held in memory on the `Index` (`Node.Status`), leaving nothing on disk. Absent binary or non-repository degrades quietly to no status and no diff. `-no-git` disables it. `/api/diff?path=<rel>` returns the unified diff for a file (`{"path","diff","available"}`), `/api/gutter?path=<rel>` returns per-line change ranges for the file view, and `/api/meta` reports a `git` boolean.
8. DOM Virtualization Frontend (`web/app.js`): Custom ~60-row DOM virtualization with offscreen canvas text measuring and `requestAnimationFrame` render throttling.
9. Markdown Preview (`markdown.go`, `web/src/markdown.js`): Renders `.md` and `.markdown` files with goldmark for a preview that opens by default and switches to the source with the Preview / Source control in the tab bar or Alt+M. See Markdown Preview below.
