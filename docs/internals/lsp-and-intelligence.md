...
2. **On-Demand Spawning**: A language server process is launched *only* when the user opens or queries a file matching its registered file extensions.
3. **Graceful Fallback**: If a server is missing, crashes, or fails initialization, the system falls back to instant regex-based symbol definitions without displaying error dialogs.
...
2. The `Origin` header must exactly match the `Host` header.
3. The `Host` header must resolve strictly to `127.0.0.1`, `[::1]`, or `localhost`.
4. Commands are pulled strictly from the internal `lspRegistry`—the client request cannot provide or modify the command string.
