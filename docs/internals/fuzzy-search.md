...
1. **Pass 1 (Forward Scan)**:
   - Evaluates whether every rune in query `q` exists sequentially in `e.lower`.
   - Records the index `end` where the final rune was located.
   - If the scan reaches the end of the path without exhausting the query, the path is immediately rejected ($O(\text{prefix})$ fast rejection).

2. **Pass 2 (Backward Scan from `end`)**:
   - Starting from `end`, scans backward toward 0, matching query characters in reverse order.
   - Because it starts from the earliest valid termination point and moves backward, it naturally converges on the tightest possible cluster of matching runes.
   - Matched byte offsets are appended to a reusable slice `pos` and flipped in-place.
...
2. Each worker allocates a local result buffer and evaluates its chunk independently without lock contention.
3. Once all workers finish, top candidate slices are merged and sorted in-place.
4. The sorted results are capped at `limit` (typically 50 results) and returned to the client as JSON.
