...
| **Rose Pine** | `rose-pine` | dark | Rosé Pine Soho vibes |
...
1. Copy an existing theme file:
   ...
   ```
2. Update the selector to match the new ID:
   ```css
   :root[data-theme="midnight"] {
   ```
3. Set the metadata properties:
   ```css
   --theme-name: "Midnight";
   color-scheme: dark;
   ```
4. Define the required color tokens (see Token Reference below).
5. Preview live without recompiling Go code by running px0 in dev mode:
   ```bash
   go run . -dev . .
   ```
6. Run unit tests to verify theme conformance:
   ```bash
   go test ./...
   ```
