...
2. **DOM Update**: `#rows.innerHTML` is updated with the visible window.
3. **`restoreSelection()`**: Uses a `TreeWalker` over the newly mounted rows to find the exact matching text offsets and calls `window.getSelection().setBaseAndExtent()`.
...
2. **Native Copy Integrity**: Selecting text never accidentally copies caret artifacts.
3. **Zero Interaction Conflicts**: Mouse drag selections never fight with caret repaints.
