...
| `<aside id="side">` | Collapsible sidebar containing panels: `#panel-files` (tree), `#panel-search`, `#panel-outline`. Its `.panel-head` shows the workspace name (`#root-name`), a `.ro-badge` marking px0 as read-only, and the re-index button (`#btn-reindex`). Its `.side-foot` holds the px0 logo, the version (`#st-ver`), links to GitHub, and the theme button (`#btn-theme`). |
...
| `web/src/tabs.js` | `openFile()`, `closeTab()`, `switchTab()`, `drawTabs()`, `drawCrumbs()`, `showImage()`. `reopenClosedTab()` (Alt+Shift+T) reopens from stack of last 20 closed files. |
