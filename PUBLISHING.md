...
2. Bundles web assets and cross-compiles binaries into `dist/`.
...
1. Navigate to **Actions** → **Release** on GitHub:
...
2. Confirm artifacts on the [Releases](https://github.com/px0-ai/px0/releases) page (cross-platform binaries and `checksums.txt`).
3. Verify the installer script:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/px0-ai/px0/master/install.sh | bash
   ```
4. Verify self-update functionality:
   ```bash
   px0 --update
   ```
